...
//上架的积分产品数据渲染
function upperIntegarGoods(pageNo,business_id,store_name,name) {
    $.ajax({
        url:"../goods/upperIntegraGoodsLis.do",
        type:"post",
            ...
            if (res.code == "1"){
                ...
                for ( var i=0; i < goods.length;i++){
                    ...
                    goodsHtml += '<tr>' +
                                    ...
                                    '<td>' +
                                        '<img class="img1" src="'+img+'" style="width: 50px;"/>'+
                                    '</td>'+
                                    '<td class="operation">' +
...
//下架的积分产品数据渲染
function lowerIntegarGoods(pageNo,business_id,store_name,name) {
    $.ajax({
        url:"../goods/lowerIntegraGoodsLis.do",
        type:"post",
        data:{pageNo:pageNo,business_id:business_id,store_name:store_name,name:name},
        beforeSend:function(){
            $(".goods").append("<td class='xuan'>loading...</td>");//显示加载动画
...
$(document).on("click",".lower",function () {
    ...
    layer.confirm("我难道违规了，您因此忍心要让我下架？",{
        ...
        $.ajax({
            url:"../goods/updateIntegarGoodsStatus.do",
            type:"post",
