...
<head>
    ...
    <title>诚亿森管理系统总后台 - 上架的积分产品列表</title>
...
<script src="${pageContext.request.contextPath}/statics/js/myjs/integarGoods.js"></script>
...
<script type="text/javascript">
    ...
    $(document).on("click",".a_name",function () {
        ...
        upperIntegarGoods("",bid,"","");
    });
    //输入框搜索
    $(document).on("click",".search",function () {
        var store_name = $("#store_name").val();
        var gname = $("#gname").val();
        upperIntegarGoods(1,"",store_name,gname);
    });
    //点击页数
    function funUpperIntegarGoods(){
        $("#pagecount span a").on('click',function(){
            var store_name = $("#store_name").val();
            var gname = $("#gname").val();
            var rel = $(this).attr("rel");
            if(rel){
                upperIntegarGoods(rel,"",store_name,gname);
            }
        });
    };
    $(document).on("click",".go",function () {
        ...
        var store_name = $("#store_name").val();
        var gname = $("#gname").val();
        upperIntegarGoods(jumpPage,"",store_name , gname);
    });
</script>
</body>
</html>
